...
- For each subsequent interval `[s, e]`:
  - If `s >= prev_end`: Compatible! Accept interval, update `prev_end = e`.
  - If `s < prev_end`: Overlap! Must remove one. Because the current interval ends after or at `prev_end`, greedily dropping the current interval preserves the earliest finish time. Increment removal count.
...
| Interval `[s, e]` | `prev_end` | Overlap Condition (`s < prev_end`)? | Action | Removals |
| :--- | :--- | :--- | :--- | :--- |
